...
import VishPopup from './VishPopup';
...
export default function VishFooter({ onFilterChange }: VishFooterProps) {
  ...
  return (
    <>
      <div className="fixed bottom-0 left-0 right-0 z-[100] text-white py-1 px-6 bg-primary z-50">
        <div className="max-w-7xl mx-auto flex flex-wrap justify-between items-center gap-4 p-2">
          ...
          <div className="flex items-center gap-4 px-4 py-2">
            <button 
              className="flex items-center justify-center w-12 h-12 bg-[#4C5F79] rounded-md hover:bg-gray-400 transition-colors"
      ...
      {showPopup && (
        <VishPopup text={wishText} onClose={handleClosePopup} />
      )}
    </>
  );
}